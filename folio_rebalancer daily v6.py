...
- Portfolio optimization methods (Equal Weight, Hierarchical Risk Parity, Hybrid HRP)
...
class Config:
    ...
    # Hybrid HRP settings
    DEFAULT_MIN_RETURN = 0.15  # Default target annual return of 15%
    DEFAULT_LAMBDA = 1.0  # Default penalty term for deviating from HRP. The parameter λ (lambda) controls the trade-off: a higher λ forces the solution to stick closer to the HRP allocation, while a lower λ prioritizes minimizing variance.
    DEFAULT_MAX_DEVIATION = 0.5  # Default maximum relative deviation from HRP weights
...
class CashAllocationManager:
    ...
    def determine_cash_allocation(
        ...
        When use_manual is False, runs all other strategies and prints results for comparison.

        Returns:
            Tuple of (cash_to_invest, cash_to_withdraw, cash_pct) for the active strategy
        """
        # Store results from all strategies for comparison
        strategy_results = {}

        # Run manual strategy if enabled
        if self.use_manual:
            print("\n" + "=" * 60)
            print("MANUAL STRATEGY")
            print("=" * 60)
            result = self.manual_strategy.calculate_allocation(
                stocks_list, cash_available, stocks_weights
            )
            strategy_results["manual"] = result
            print(
                f"Manual Strategy Result: Invest ${result[0]:,.2f}, Withdraw ${result[1]:,.2f}, Cash % {result[2]:.2%}"
            )
            return result

        # If not using manual, run all other strategies and print results
        print("\n" + "=" * 60)
        print("CASH ALLOCATION STRATEGY COMPARISON")
        print("=" * 60)

        # Run Fear & Greed strategy
        if self.use_fear_greed:
            print("\n" + "-" * 40)
            print("FEAR & GREED STRATEGY")
            print("-" * 40)
            result = self.fear_greed_strategy.calculate_allocation(
                stocks_list, cash_available, stocks_weights
            )
            strategy_results["fear_greed"] = result
            print(
                f"Fear & Greed Result: Invest ${result[0]:,.2f}, Withdraw ${result[1]:,.2f}, Cash % {result[2]:.2%}"
            )

        # Run Drawdown strategy
        if self.use_drawdown:
            print("\n" + "-" * 40)
            print("DRAWDOWN TRIGGER STRATEGY")
            print("-" * 40)
            result = self.drawdown_strategy.calculate_allocation(
                stocks_list, cash_available, stocks_weights
            )
            strategy_results["drawdown"] = result
            print(
                f"Drawdown Strategy Result: Invest ${result[0]:,.2f}, Withdraw ${result[1]:,.2f}, Cash % {result[2]:.2%}"
            )
        ...
        if self.use_mean_reversion:
            ...
            print("-" * 40)
            result = self.mean_reversion_strategy.calculate_allocation(
                stocks_list, cash_available, stocks_weights
            )
            strategy_results["mean_reversion"] = result
            print(
                f"Mean Reversion Strategy Result: Invest ${result[0]:,.2f}, Withdraw ${result[1]:,.2f}, Cash % {result[2]:.2%}"
            )
        ...
        for strategy_name, result in strategy_results.items():
            print(
                f"{strategy_name.replace('_', ' ').title()}: Invest ${result[0]:,.2f}, Withdraw ${result[1]:,.2f}, Cash % {result[2]:.2%}"
            )

        # Determine which strategy result to use and print it
        selected_result = None
        selected_strategy = None

        if self.use_fear_greed and "fear_greed" in strategy_results:
            selected_result = strategy_results["fear_greed"]
            selected_strategy = "Fear & Greed"
        elif self.use_drawdown and "drawdown" in strategy_results:
            selected_result = strategy_results["drawdown"]
            selected_strategy = "Drawdown"
        elif self.use_mean_reversion and "mean_reversion" in strategy_results:
            selected_result = strategy_results["mean_reversion"]
            selected_strategy = "Mean Reversion"
        else:
            # Fallback if no strategies are enabled
            print("\nNo strategies enabled, returning zero allocation")
            return 0.0, 0.0, 0.0

        print(f"\nUsing {selected_strategy} Strategy result")

        # Plot portfolio value after printing all results
...
class PortfolioOptimizer:
    ...
    @staticmethod
    def hybrid_hrp_optimization(
        stocks_list: List[str],
        min_return: float,
        lambda_val: float,
    ) -> Tuple[Dict[str, float], Dict[str, float]]:
        """
        Calculate weights using a hybrid of HRP and MVO to meet a minimum return.

        Args:
            stocks_list: List of stock symbols.
            min_return: The target minimum annualized return.
            lambda_val: Penalty for deviating from HRP weights.

        Returns:
            Tuple of (original_hrp_weights, optimized_weights).
        """
        print(
            f"Running Hybrid HRP with min return: {min_return:.2%} and lambda: {lambda_val}"
        )

        # 1. Fetch data (same as standard HRP)
        prices = DataFetcher.fetch_price_history(
            stocks_list, Config.HRP_DATA_PERIOD, Config.HRP_DATA_INTERVAL
        )

        # 2. Calculate expected returns (mu) and covariance (S)
        mu = expected_returns.mean_historical_return(prices)
        S = CovarianceShrinkage(prices).ledoit_wolf()

        # 3. Get the baseline HRP weights (w_HRP)
        hrp_weights_dict = PortfolioOptimizer.hierarchical_risk_parity_optimization(
            stocks_list
        )
        # Ensure the hrp_weights array is in the same order as the tickers
        hrp_weights_array = np.array(
            [hrp_weights_dict[ticker] for ticker in stocks_list]
        )

        # 4. Set up the constrained optimization problem
        # Define the custom objective function
        # Objective = w^T * S * w + lambda * ||w - w_HRP||^2
        weights = cp.Variable(len(stocks_list))
        variance = cp.quad_form(weights, S)
        hrp_distance = cp.sum_squares(weights - hrp_weights_array)
        objective = cp.Minimize(variance + lambda_val * hrp_distance)

        # Calculate expected return of HRP weights for comparison
        hrp_expected_return = np.dot(hrp_weights_array, mu)
        print(f"HRP expected return: {hrp_expected_return:.2%}")
        print(f"Target minimum return: {min_return:.2%}")

        # Define constraints
        constraints = [
            cp.sum(weights) == 1,  # sum(w) = 1
            weights >= 0,  # w >= 0 (no shorting)
            weights.T @ mu >= max(hrp_expected_return, min_return),  # w^T * mu >= R_min
        ]
        ...
        if prob.status != "optimal":
            print(
                f"Warning: Solver did not find an optimal solution. Status: {prob.status}"
            )
            if prob.status == "infeasible":
                print(
                    "The optimization problem is infeasible - cannot achieve the target return with current constraints."
                )
                print(
                    "Consider: 1) Lowering the target return, 2) Increasing max_deviation, 3) Allowing shorting"
                )

        # 6. Extract and clean the final weights
        raw_weights = weights.value

        # Round weights less than 0.0001 to zero and normalize
        cleaned_weights_array = np.where(raw_weights < 0.0001, 0, raw_weights)
        # Renormalize so weights sum to 1
        if np.sum(cleaned_weights_array) > 0:
            cleaned_weights_array = cleaned_weights_array / np.sum(
                cleaned_weights_array
            )

        # Calculate optimized expected return using cleaned weights
        optimized_expected_return = np.dot(cleaned_weights_array, mu)
        print(f"Optimized expected return: {optimized_expected_return:.2%}")

        # Create final weights dictionary
        final_weights = dict(zip(stocks_list, cleaned_weights_array))

        return hrp_weights_dict, final_weights
...
class PortfolioRebalancer:
    ...
    def __init__(
        ...
        min_return_target: float = Config.DEFAULT_MIN_RETURN,
        lambda_param: float = Config.DEFAULT_LAMBDA,
    ):
    ...
    def rebalance_portfolio(
        ...
        Args:
            ...
            stocks_list: List of stock symbols
            cash_to_invest: Cash to invest (if None, will calculate)
            cash_to_withdraw: Cash to withdraw (if None, will calculate)
            cash_pct: Target cash percentage (if None, will calculate)

        Returns:
            DataFrame with rebalancing recommendations
        """
        # Determine cash allocations if not provided
        if cash_to_invest is None or cash_to_withdraw is None or cash_pct is None:
        ...
        if self.optimization_method == "Equal Weight":
            ...
            optimized_weights = None
        elif self.optimization_method == "Hybrid HRP":
            target_weights, optimized_weights = (
                PortfolioOptimizer.hybrid_hrp_optimization(
                    stocks_list,
                    self.min_return_target,
                    self.lambda_param,
                )
            )
        else:
        ...
        # Assign target weights to DataFrame (use original HRP weights for Hybrid HRP)
        data_manager.df["target_weight"] = data_manager.df["Symbol"].apply(
            lambda x: target_weights.get(x, 0)
        )

        # Add optimized weights column for Hybrid HRP
        if optimized_weights is not None:
            data_manager.df["optimized_weight"] = data_manager.df["Symbol"].apply(
                lambda x: optimized_weights.get(x, 0)
            )
    ...
    def _calculate_trades(
        ...
        # Use optimized weights for calculations if available (Hybrid HRP), otherwise use target weights
        weights_for_calculation = (
            data_manager.df["optimized_weight"]
            if "optimized_weight" in data_manager.df.columns
            else data_manager.df["target_weight"]
        )

        # Target dollar value per ticker on the adjusted total
        data_manager.df["target_value"] = weights_for_calculation * new_total_value

        # Calculate raw trade dollars
        raw_trade = (
            weights_for_calculation * new_total_value - data_manager.df["Current Value"]
        )
...
class PortfolioReporter:
    ...
    def print_summary(
        ...
        gain_loss_pct = (
            (total_gain_loss / current_value_total)
    ...
    def save_results(
        ...
        output_filename = os.path.join(
            output_dir,
            f"Portfolio_Rebalance_{optimization_method.replace(' ', '_')}_{date}_{timestamp}.csv",
        )
...
def main():
    ...
    # === User Configuration ===
    date = "Sep-04-2025"
    # Options: "Equal Weight", "Hierarchical Risk Parity", "Hybrid HRP"
    optimization_method = "Hybrid HRP"
    rebalancing_mode = True

    # Hybrid HRP parameters (used only when optimization_method = "Hybrid HRP")
    min_return_target = 0.3  # Config.DEFAULT_MIN_RETURN  # 20% is more realistic
    lambda_param = 1  # Config.DEFAULT_LAMBDA  # Controls how close to HRP we stay, higher values mean closer to HRP

    # Cash allocation settings
    use_manual_cash_settings = False
    use_fgn = True
    ...
    # Get cash allocation for reporting (before rebalancing)
    current_weights = data_manager.get_current_weights()
    cash_to_invest, cash_to_withdraw, cash_pct = (
    ...
    # Print summary before rebalancing starts
    ...
    # Perform rebalancing
    result_df = rebalancer.rebalance_portfolio(
        data_manager, stocks_list, cash_to_invest, cash_to_withdraw, cash_pct
    )
