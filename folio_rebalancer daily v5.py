...
class Config:
    ...
    # Hybrid HRP settings
    DEFAULT_MIN_RETURN = 0.15  # Default target annual return
    ...
    DEFAULT_MAX_DEVIATION = 0.25  # Default relative maximum deviation from HRP weights. i.e. hrp weight 0.10 can change between 0.10 * 0.95 and 0.10 * 1.05
...
class PortfolioOptimizer:
    ...
    def hybrid_hrp_optimization(
        ...
        """
        print(
            f"Running Hybrid HRP with min return: {min_return:.2%}, lambda: {lambda_val} and max deviation: {max_deviation:.2%}"
        )
        ...
        # Define the bounds for each stock (relative deviation)
        # Ensure bounds don't go below 0
        lower_bounds = np.maximum(hrp_weights_array * (1 - max_deviation), 0)
        upper_bounds = np.minimum(hrp_weights_array * (1 + max_deviation), 1)

        # Define constraints
        constraints = [
            cp.sum(weights) == 1,  # sum(w) = 1
            weights >= 0,  # w >= 0 (no shorting)
            weights.T @ mu >= min_return,  # w^T * mu >= R_min
            weights >= lower_bounds,  # w >= lower_bounds
            weights <= upper_bounds,  # w <= upper_bounds
        ]
        ...
        if prob.status != "optimal":
            ...
            if prob.status == "infeasible":
                print(
                    "The optimization problem is infeasible with current constraints."
                )
                return (
                    hrp_weights_dict,
                    hrp_weights_dict,
                )  # Return original weights if infeasible

        # 6. Extract the final weights
        # Use raw weights directly to preserve bounds
        # Note: clean_weights() can violate bounds by rounding small weights to zero
        raw_weights = weights.value
        cleaned_weights = dict(zip(stocks_list, raw_weights))

        return hrp_weights_dict, cleaned_weights
...
class PortfolioRebalancer:
    ...
    def __init__(
        ...
        max_deviation: float = Config.DEFAULT_MAX_DEVIATION,
    ):
...
def main():
    ...
    min_return_target = Config.DEFAULT_MIN_RETURN  # 10% target return
    lambda_param = 1.0  # Config.DEFAULT_LAMBDA  # Controls how close to HRP we stay, higher values mean closer to HRP
    max_deviation = Config.DEFAULT_MAX_DEVIATION  # 5% max relative deviation
